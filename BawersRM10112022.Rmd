...
library(gmodels)
library(caret)
# Next up, we can create a CrossTable
CrossTable(x = yyes_prediction, y = tele_test$yyes, prop.chisq = FALSE)

# Let's also create a confusionMatrix
...
## Next Up: Improving Our ANN Model
...
```{r}
# We have thankfully already cleaned the data up. 
# Additionally, Logistic Regression forces dummy variables and doesn't need normalized values.
# But let's just use a dummy version instead so that we at least have some dummies, being tele_modelmatrix

set.seed(12345)
tele_random_log <- tele[sample(nrow(tele)),]

# again, we can create a training and test set now...

## factor default has new levels yes --> issue because our train_log didn't have the value "yes", but our tele_test_log did...
tele_train_log <- tele_random_log[1:8238,]
tele_test_log <- tele_random_log[8239:41188,]
summary(tele_train_log)

cor(tele_train_log$age, tele_train_log$campaign)
# Let's remove the y column
...
## Error: factor default has new levels yes. This issue arises because our train_log didn't have the value "yes", but our tele_test_log did...
...
# !!!!!!!!!!! Technically, we could just remove the default column in both our regression and thus our predict, since we can already see that it's not significant. Let's do that! Alternatively, we could deliberately include a row with default = yes in our train set, but that would skew our comparison relative to ANN and KNN, so that has been avoided. 
...
CrossTable(binarypredictionlog, tele_test_log_labels, prop.chisq = FALSE)
...
# Improving our Linear Regression
...
CrossTable(binarypredictionlog2, tele_test_log_labels, prop.chisq = FALSE)

confusionMatrix(data = binarypredictionlog2, reference = tele_test_log_labels, positive = "1")
