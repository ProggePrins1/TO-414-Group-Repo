...
client_clusters <- kmeans(tele_train, 5)

client_clusters$size
# Thankfully, there's not a cluster that's overtly large or small here, which would show that k should have been set differently (if the group were too large, we might have needed another k. If it were too small, our clusters would be too, so that we could actually remove some k's).

client_clusters$centers

# Cluster 1: June, Rich (high cons.price.idx)
# Cluster 2: Single, 
# Cluster 3: Married,
# Cluster 4: Other?
# Cluster 5: Other?

tele_train$cluster <- client_clusters$cluster
tele_train$cluster <- as.factor(tele_train$cluster)

success_by_segm <- tapply(tele_train$y, tele_train$cluster, mean, na.rm = T)
success_by_segm
``` 

```{r}
# First Improvement - Checking the Number of Clusters

# Distance within clusters:
kmeans.wss.k <- function(tele_train, k){
  km = kmeans(tele_train, k)
  return (km$tot.withinss)
}

# k = 5
kmeans.wss.k(tele_train, 5)
# k = 6
kmeans.wss.k(tele_train, 6)

kmeans.dis <- function(data_z, maxk){
  dis=(nrow(data_z)-1)*sum(apply(data_z,2,var))
  dis[2:maxk]=sapply (2:maxk, kmeans.wss.k, tele_train=tele_train)
  return(dis) }

maxk = 30
dis = kmeans.dis(tele_train, maxk)

plot(1:maxk, dis, type = 'b', xlab="Number of Clusters", ylab="Distortion", col="black")

# So, maybe four clusters suffice? Let's check the intra-cluster similarity relative to 5.

kmeans.wss.k(tele_train, 4)
# That's only 932 more than k = 5. Let's go for it!
```
