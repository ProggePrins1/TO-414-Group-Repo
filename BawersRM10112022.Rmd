...
tele_test5_log <- cluster5[(nrow(tele_train5)+1):nrow(cluster4),]
...
# Unfortunately, we get an error here. It appears to be the case that for our tele_train5_log, we have a predictor variable which only has one value, causing the regression to fail. Let's find and fix this issue.
...
binarypredictionlogt4 <- ifelse(predict(predictionmodelt2, newdata = tele_test4_log, type = "response") < 0.5, 0, 1)

binarypredictionlogt5 <- ifelse(predict(predictionmodelt2, newdata = tele_test5_log, type = "response") < 0.5, 0, 1)
...
Technically, we could just remove the month column in both our regression and thus our predict, since we can already see that it's not significant. Let's do that! Alternatively, we could deliberately include an observation with default = yes in our train set, but that would skew our comparison relative to ANN and KNN. Thus, doing this is ill-advised.
...
improvedpredictionmodelt2 <- glm(y ~. - education - age -day_of_week -emp.var.rate
                                 - cons.price.idx - cons.conf.idx - nr.employed -pdaysdummy
                                 - campaign -previous -euribor3m -contact -default
                                 - loan - housing, data = tele_train2_log, family = "binomial")
...
improvedpredictionmodelt4 <- glm(y ~ . -month -job -age -marital - default
                                 - housing - loan - campaign - previous -emp.var.rate
                                 - cons.price.idx - cons.conf.idx - pdaysdummy, data = tele_train4_log, family = "binomial")
